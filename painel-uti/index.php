...
<body>
    ...
    <script>
        ...
        function loadDashboard() {
            ...
            // Buscar dados da API
            Promise.all([
                fetch(`../api.php?action=dashboard_data&month=${new Date().toISOString().substring(0, 7)}`).then(r => r.json())
            ])
            .then(([dashboardData]) => {
        ...
        function updateDashboard(dashboardData) {
            ...
            const totalActivities = dashboardData.totals.total_items;
            const completedActivities = dashboardData.totals.total_completed;
            const notCompletedActivities = dashboardData.totals.total_not_completed;
            const pendingActivities = totalActivities - completedActivities - notCompletedActivities;
            const completionRate = dashboardData.totals.completion_percentage;
